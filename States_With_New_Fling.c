...
int backDistance = 25;
int backFinished = 0;
task moveBackForShot()
{
	int oldFinish = 0;
	while(1)
	{
		if(backFinished != oldFinish)
		{
			backFinished = 0;
			moveForwardsInches(-backDistance);
			backFinished = 1;
		}
		oldFinish = backFinished;
	}
}
void flingShot(int dist, int time)
{
	backDistance = dist;
	startTask(moveBackForShot);
	backFinished = 1;
	while(SensorValue[liftPot] > LIFT_FLING){}
	clawopen();
	while(!liftdone){}
	while(backFinished == 0);
	wait1Msec(800);
	stopTask(moveBackForShot);
}
