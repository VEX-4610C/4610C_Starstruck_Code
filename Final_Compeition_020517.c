...
void moveForwardsInches(int inches) // Y Axis, Pos=Forward Neg=Backwards
{
	...
	{
		//motor[backLeft] = -1 * (30+ ((abs(nMotorEncoder[backLeft])-abs(wheeldegs))*P_Factor))*SIGN(wheeldegs);
		...
		//motor[frontRight] = -1 * (30+ ((abs(nMotorEncoder[backLeft])-abs(wheeldegs))*P_Factor))*SIGN(wheeldegs);
		motor[backLeft] = 127*SIGN(wheeldegs)*-1;
		motor[backRight] = -127*SIGN(wheeldegs)*-1;
		motor[frontRight] = 127*SIGN(wheeldegs)*-1;
		motor[frontLeft] = -127*SIGN(wheeldegs)*-1;
	}
...
void moveBackwardsInches(int inches) // Y Axis, Pos=Forward Neg=Backwards
{
	wheeldegs = (inches/12.566)*360; // Calculating Wheel Degrees
	float P_Factor = 0.75; // Factor for P Controller
	nMotorEncoder[backLeft] = 0; // Reset Encoder
